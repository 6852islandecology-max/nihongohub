  ...
  function resolvePlan(chip) {
    fetch(BASE + 'api/public-config').then(function (r) { return r.ok ? r.json() : null; }).then(function (cfg) {
      ...
      loadSDK().then(function (sdk) {
        ...
        // Passthrough lock: this client shares the default storageKey with index.html's client,
        // and supabase-js v2's navigator.locks coordination deadlocks when two clients contend
        // for the same lock (auth hangs forever with no error). Run ops without the lock.
