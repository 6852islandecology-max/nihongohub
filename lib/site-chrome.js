...
(function () {
  ...
  // 2026-08-23: 交換する物を「毎週のメール」から「いま渡せる印刷物」に変更 (blog/blog-quiz.js と同じ)。
  var FIELD_KIT = '/downloads/japan-craft-collectible-field-kit.pdf';
  var NL_HTML =
    '<form class="nh-foot-nl" novalidate>' +
    '<div class="nh-foot-nl-h">Free: the Japan Craft &amp; Collectible Field Kit</div>' +
    ...
    '<button type="submit">Send me the field kit →</button></div>' +
    '<div class="nh-foot-nl-s">8 printable pages: craft towns, what to check before you buy, the five free collections, 18 shop phrases. Then one prefecture a week. Unsubscribe anytime.</div>' +
    '</form>';
  function wireFooterNl(form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch('/api/public-config').then(function (r) { return r.json(); }).then(function (cfg) {
        ...
        var p = el('div', 'nh-foot-nl-s');
        p.innerHTML = (r.status === 409 ? 'You’re already on the list — thank you. ' : 'You’re in. ') +
          '<a href="' + FIELD_KIT + '" target="_blank" rel="noopener"><b>Download the Field Kit (PDF, 8 pages) →</b></a>' +
          ' The first prefecture letter arrives this week.';
  ...
  function funnelBeacon() {
    ...
    try {
      ...
      // 再訪セッション判定 (2026-08-28): blog/blog-quiz.js と同じロジック。
      // セッションに1回だけ判定して sessionStorage に固定 (同日回遊で消えないように)。
      var isRet = '0';
      try {
        var d0 = new Date();
