...
(function () {
  ...
  function setChip(chip, label, state) {
    ...
    chip.href = BASE + (state === 'paid' ? 'dashboard.html' : 'index.html#pricing');
  }
    ...
    try {
      ...
      document.addEventListener('click', function (ev2) {
        try {
          ...
          if (!a) return;
          var key = (a.getAttribute('data-aff') || '').toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 24);
          if (key) send('aff_' + key);
        } catch (e3) {}
      }, true);
    } catch (e) {}
  }
