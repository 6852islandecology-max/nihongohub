...
(function () {
  ...
  async function draw(canvas) {
    ...
    var cx = W / 2, cyTop = 300, panelH = 480;
    // panel
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      if (meta) {
        ...
        // item names under the badge (EN + JA), shrink-to-fit 230px
        var nameEn = (meta.name && meta.name.en) || '';
        var nameJa = (meta.name && meta.name.ja) || '';
        var nx = bx + bs / 2, maxW = 230;
        ctx.textAlign = 'center';
        if (nameEn) {
          var fs = 22;
          ctx.font = '700 ' + fs + 'px "DM Sans", sans-serif';
          while (fs > 14 && ctx.measureText(nameEn).width > maxW) { fs--; ctx.font = '700 ' + fs + 'px "DM Sans", sans-serif'; }
          ctx.fillStyle = '#fdf6e3'; ctx.fillText(nameEn, nx, by + bs + 30);
        }
        if (nameJa) {
          var fj = 18;
          ctx.font = '500 ' + fj + 'px "Noto Sans JP", sans-serif';
          while (fj > 12 && ctx.measureText(nameJa).width > maxW) { fj--; ctx.font = '500 ' + fj + 'px "Noto Sans JP", sans-serif'; }
          ctx.fillStyle = '#c9b58a'; ctx.fillText(nameJa, nx, by + bs + 56);
        }
      }
    }
  ...
  function open() {
    ...
    ov.innerHTML =
      ...
      '</div>' +
      // Desktop-friendly per-SNS buttons. Web intents cannot attach an image file, so:
      // X/Threads — copy the CARD IMAGE to the clipboard, open the compose tab, user pastes (Ctrl+V).
      // Instagram — no web compose intent: download image + copy caption + open instagram.com.
      // Facebook — sharer with quote (its card uses the page OGP; image can be added manually).
      '<div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center;margin-top:10px">' +
      '<button class="nh-sc-sns" data-sns="x" style="flex:1;min-width:72px;background:#000;color:#fff;border:1px solid #3d2f1f;border-radius:8px;padding:10px 6px;font:700 13px sans-serif;cursor:pointer">𝕏</button>' +
      '<button class="nh-sc-sns" data-sns="threads" style="flex:1;min-width:72px;background:#1a1a1a;color:#fff;border:1px solid #3d2f1f;border-radius:8px;padding:10px 6px;font:700 13px sans-serif;cursor:pointer">Threads</button>' +
      '<button class="nh-sc-sns" data-sns="instagram" style="flex:1;min-width:72px;background:linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888);color:#fff;border:none;border-radius:8px;padding:10px 6px;font:700 13px sans-serif;cursor:pointer">Instagram</button>' +
      '<button class="nh-sc-sns" data-sns="facebook" style="flex:1;min-width:72px;background:#1877f2;color:#fff;border:none;border-radius:8px;padding:10px 6px;font:700 13px sans-serif;cursor:pointer">Facebook</button>' +
      '<button class="nh-sc-sns" data-sns="copy" style="flex:1;min-width:72px;background:transparent;color:#e8a020;border:1px solid #e8a020;border-radius:8px;padding:10px 6px;font:700 13px sans-serif;cursor:pointer">📋 Copy</button>' +
      '</div>' +
      '<p style="color:#6f5d44;font:12px sans-serif;margin:8px 0 0">X / Threads: your card is copied — press Ctrl+V (⌘V) in the post to attach it.</p>' +
    ...
    // Copy the rendered card image itself to the clipboard (Chrome/Edge/Safari).
    // Compose pages prefill the text via the intent URL, so the user only pastes the image.
    async function copyCardImage() {
      if (!navigator.clipboard || typeof ClipboardItem === 'undefined') return false;
      try {
        var blob = await canvasToBlob(cv); if (!blob) return false;
        await navigator.clipboard.write([new ClipboardItem({ 'image/png': blob })]);
        return true;
      } catch (e) { return false; }
    }
    var SNS_URL = {
      x:        function (t) { return 'https://twitter.com/intent/tweet?text=' + encodeURIComponent(t); },
      threads:  function (t) { return 'https://www.threads.net/intent/post?text=' + encodeURIComponent(t); },
      facebook: function (t) { return 'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent('https://www.nihongo-hub.com/rpg.html') + '&quote=' + encodeURIComponent(t); },
      instagram: function () { return 'https://www.instagram.com/'; }
    };
    ov.querySelectorAll('.nh-sc-sns').forEach(function (b) {
      b.onclick = async function () {
        var sns = b.dataset.sns;
        var text = shareText(document.getElementById('nh-sharecard-tags').value);
        if (sns === 'copy') {
          try { await navigator.clipboard.writeText(text); note.textContent = 'Caption copied 📋 — use ⬇ Download for the image.'; }
          catch (e) { note.textContent = 'Copy failed — select & copy the caption manually.'; }
          return;
        }
        if (sns === 'x' || sns === 'threads') {
          var copied = await copyCardImage();
          if (!copied && !dlOnce) await downloadCard();
          window.open(SNS_URL[sns](text), '_blank', 'noopener');
          note.textContent = copied
            ? 'Your card is on the clipboard — press Ctrl+V (⌘V) in the post to attach it 🎉'
            : 'Image downloaded — attach it to your post (text is prefilled).';
          return;
        }
        if (sns === 'instagram') {
          try { await navigator.clipboard.writeText(text); } catch (e) {}
          if (!dlOnce) await downloadCard();
          window.open(SNS_URL.instagram(), '_blank', 'noopener');
          note.textContent = 'Image downloaded & caption copied — create a post on Instagram and add them 🎉';
          return;
        }
        // facebook
        try { await navigator.clipboard.writeText(text); } catch (e) {}
        if (!dlOnce) await downloadCard();
        window.open(SNS_URL.facebook(text), '_blank', 'noopener');
        note.textContent = 'Caption copied & image downloaded — add the image to your Facebook post.';
      };
    });
