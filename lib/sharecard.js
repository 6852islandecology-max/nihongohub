...
(function () {
  var W = 1080, H = 1080;
  var DEFAULT_TAGS = '#NihongoHub #LearnJapanese #JLPT #日本語';
  ...
  async function draw(canvas) {
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      ctx.fillStyle = 'rgba(0,0,0,.35)'; ctx.strokeStyle = '#3d2f1f'; ctx.lineWidth = 3;
    ...
    var stats = [['STAGE', stage], ['LEVEL', String(lv)], ['XP', String(xp)], ['BEST 🔥', String(best)], ['TITLES', String(words)]];
  ...
  function shareText(tags) {
    ...
    var ja = (T && comp) ? T.composeDisplay(comp, 'ja') : '';
    var xp = parseInt(get('nh_quiz_xp', '0'), 10) || 0;
    var head = ja ? ('My NihongoHub title: 「' + ja + '」') : 'My NihongoHub adventure';
    return head + ' · ' + stageFromXp(xp) + ' · ' + xp + ' XP\n' + (tags || DEFAULT_TAGS) + '\nhttps://www.nihongo-hub.com/rpg.html';
  }
  ...
  function open() {
    ...
    ov.innerHTML =
      '<div style="background:#1a1008;border:2px solid #e8a020;border-radius:14px;padding:18px;max-width:440px;width:100%;max-height:92vh;overflow:auto;text-align:center">' +
      '<canvas id="nh-sharecard-cv" style="width:100%;border-radius:10px;background:#160f08"></canvas>' +
